...
#### 🍪 Cookie Import (Recommended for Servers)
Perfect for headless servers or remote machines. No browser needed on the server!

```
"Import NotebookLM cookies for authentication"
```
...
1. Login to NotebookLM on any device with a browser
2. Export cookies using a browser extension (EditThisCookie, Cookie-Editor)
3. Paste them into the terminal

📖 **[Full Cookie Import Guide](COOKIE_IMPORT_GUIDE.md)**

#### 🌐 Direct Browser (For Local Machines)
Opens a browser window for you to login directly:
